...
export async function POST(request: NextRequest) {
  try {
    const { nama, deskripsi, kategori_id, jumlah, status, foto, harga_per_hari, barcode } = await request.json();

    const result = await pool.query(
      `INSERT INTO alat (nama, deskripsi, kategori_id, jumlah, jumlah_tersedia, status, foto, harga_per_hari, barcode) 
       VALUES ($1, $2, $3, $4, $4, $5, $6, $7, $8) 
       RETURNING *`,
      [nama, deskripsi || null, kategori_id || null, jumlah, status || 'tersedia', foto || null, harga_per_hari || 0, barcode || `ALT-${Math.random().toString(36).substr(2, 9).toUpperCase()}`]
    );
